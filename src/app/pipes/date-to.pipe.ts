...
export class DateToPipe implements PipeTransform {
  ...
  transform(value: any): any {
    ...
    if (deltaSeconds < 180) {
      return `${deltaSeconds} ${BaseComponent.translate('seconds')}
      `;
    }
    const deltaMinutes = Math.round(deltaSeconds / 60);
    if (deltaMinutes < 60) {
      return `${deltaMinutes}  ${BaseComponent.translate('minutes')}`;
    }
    const deltaHours = Math.round(deltaMinutes / 60);
    if (deltaHours < 24) {
      return `${deltaHours}  ${BaseComponent.translate('hours')}`;
    }
    const deltaDays = Math.round(deltaHours / 24);
    if (deltaDays < 60) {
      return `${deltaDays}  ${BaseComponent.translate('days')}`;
    }
    const deltaWeeks = Math.round(deltaDays / 7);
    if (deltaWeeks < 20) {
      return `${deltaWeeks}  ${BaseComponent.translate('weeks')}`;
    }
    const deltaMontsh = Math.round(deltaWeeks / 4.2);
    return `${deltaMontsh}  ${BaseComponent.translate('months')}`;
  }
}
