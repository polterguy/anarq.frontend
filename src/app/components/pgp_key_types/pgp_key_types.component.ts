 ...
 */
@Component({
  selector: 'app-pgp_key_types',
  templateUrl: './pgp_key_types.component.html',
  styleUrls: ['./pgp_key_types.component.scss']
})
export class Pgp_key_typesComponent implements OnInit {
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.pgp_key_types_Get(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.pgp_key_types_count_Get(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditPgp_key_typesComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('Pgp_key_types item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Pgp_key_types item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.pgp_key_types_Delete(ids).subscribe(res => {
