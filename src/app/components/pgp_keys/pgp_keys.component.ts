...
import { EditPgp_keysComponent } from './modals/edit.pgp_keys.component';
import { PgpService } from 'src/app/services/pgp.service';

/*
 * "Datagrid" component for displaying instance of Pgp_keys
 * entities from your HTTP REST backend.
 */
@Component({
  selector: 'app-pgp_keys',
  templateUrl: './pgp_keys.component.html',
  styleUrls: ['./pgp_keys.component.scss']
})
export class Pgp_keysComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['id', 'user', 'key_id', 'type', 'fingerprint', 'created', 'expires', 'enabled', 'revoked', 'strength', 'content', 'uploaded', 'delete-instance'];
  ...
  private key_id: FormControl;
  private type: FormControl;
  private fingerprint: FormControl;
  private created: FormControl;
  private expires: FormControl;
  private enabled: FormControl;
  private revoked: FormControl;
  private strength: FormControl;
  private content: FormControl;
  private uploaded: FormControl;
  ...
  ngOnInit() {
    ...
    this.key_id = new FormControl('');
    this.key_id.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['key_id.like'] = this.key_id.value + '%';
        this.getData();
      });
    ...
    this.fingerprint = new FormControl('');
    this.fingerprint.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['fingerprint.like'] = this.fingerprint.value + '%';
        this.getData();
      });
    ...
    this.expires = new FormControl('');
    this.expires.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['expires.eq'] = this.expires.value;
        this.getData();
      });
    ...
    this.enabled.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['enabled.eq'] = this.enabled.value;
        this.getData();
      });
    this.revoked = new FormControl('');
    this.revoked.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['revoked.eq'] = this.revoked.value;
        this.getData();
      });
    this.strength = new FormControl('');
    this.strength.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['strength.eq'] = this.strength.value;
        this.getData();
      });
    ...
    this.uploaded = new FormControl('');
    this.uploaded.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['uploaded.eq'] = this.uploaded.value;
        this.getData();
      });
  }
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditPgp_keysComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('Pgp_keys item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Pgp_keys item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
