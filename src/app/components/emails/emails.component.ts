...
export class EmailsComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['id', 'type', 'signature', 'encrypted', 'content', 'delete-instance'];
  ...
  private signature: FormControl;
  private encrypted: FormControl;
  ...
  ngOnInit() {
    ...
    this.signature = new FormControl('');
    this.signature.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['signature.eq'] = this.signature.value;
        this.getData();
      });
    this.encrypted = new FormControl('');
    this.encrypted.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['encrypted.eq'] = this.encrypted.value;
        this.getData();
      });
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditEmailsComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('Emails item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Emails item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
