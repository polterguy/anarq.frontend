...
export class TranslationsComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['locale', 'key', 'content', 'delete-instance'];
  ...
  ngOnInit() {
    ...
    this.key.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['key.like'] = this.key.value + '%';
        this.getData();
      });
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.getTranslation(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.countTranslations(cloned).subscribe(res2 => {
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.deleteTranslation(ids).subscribe(res => {
