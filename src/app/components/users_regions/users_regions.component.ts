...
import { EditUsers_regionsComponent } from './modals/edit.users_regions.component';
import { UsersService } from 'src/app/services/http/users.service';

/*
 * "Datagrid" component for displaying instance of Users_regions
 * entities from your HTTP REST backend.
 */
@Component({
  selector: 'app-users_regions',
  templateUrl: './users_regions.component.html',
  styleUrls: ['./users_regions.component.scss']
})
export class Users_regionsComponent implements OnInit {
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.getUsersRegions(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.countUsersRegions(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditUsers_regionsComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('Users_regions item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Users_regions item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.deleteUsersRegions(ids).subscribe(res => {
