 ...
 */
@Component({
  templateUrl: './edit.user_status.component.html',
  styleUrls: ['./edit.user_status.component.scss']
})
export class EditUser_statusComponent {
   ...
   */
  private createColumns: string[] = ['status', 'description'];
  private updateColumns: string[] = ['status', 'description'];
  private primaryKeys: string[] = ['status'];
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.updateUserStatus(this.data.entity).subscribe(res => {
      ...
      this.service.createUserStatus(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
