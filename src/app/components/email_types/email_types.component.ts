...
export class Email_typesComponent implements OnInit {
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.getEmailType(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.countEmailTypes(cloned).subscribe(res2 => {
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.deleteEmailType(ids).subscribe(res => {
