 ...
 */
@Component({
  selector: 'app-user_status',
  templateUrl: './user_status.component.html',
  styleUrls: ['./user_status.component.scss']
})
export class User_statusComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['status', 'description', 'delete-instance'];
  ...
  ngOnInit() {
    ...
    this.status.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter['status.like'] = this.status.value + '%';
        this.getData();
      });
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditUser_statusComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('User_status item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('User_status item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
