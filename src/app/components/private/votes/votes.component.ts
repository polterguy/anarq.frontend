...
import { EditVotesComponent } from './modals/edit.votes.component';
import { MiscService } from 'src/app/services/http/misc.service';

/*
 * "Datagrid" component for displaying instance of Votes
...
export class VotesComponent implements OnInit {
  ...
  constructor(
    private httpService: MiscService,
  ...
  ngOnInit() {
    ...
    this.case = new FormControl('');
    this.case.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
    ...
    this.opinion.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['opinion.eq'] = this.opinion.value;
        this.getData();
      });
  }
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditVotesComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('Votes item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Votes item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
