 ...
 */
@Component({
  selector: 'app-users_kyc_documents',
  templateUrl: './users_kyc_documents.component.html',
  styleUrls: ['./users_kyc_documents.component.scss']
})
export class Users_kyc_documentsComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['id', 'user', 'public', 'content', 'delete-instance'];
  ...
  ngOnInit() {
    ...
    this.public = new FormControl('');
    this.public.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['public.eq'] = this.public.value;
        this.getData();
      });
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(EditUsers_kyc_documentsComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('Users_kyc_documents item successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('Users_kyc_documents item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
